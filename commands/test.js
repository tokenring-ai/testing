...
import TestingService from "../TestingService.js";
  ...
  export const description = "/test [test_name|all] - Run all or a specific test from any TestingService. Shows available tests if name is omitted.";
  ...
  export async function execute(remainder, registry) {
    ...
    if (!remainder || remainder.trim() === "all") {
      ...
      const testName = Array.from(availableTests.keys())
        .find(k => k.toLowerCase() === remainder.trim().toLowerCase());
      if (!testName) {
        chatService.errorLine("Test not found: " + remainder);
        chatService.systemLine("Available tests: " + Array.from(availableTests.keys()).join(", "));
        return;
      }
      toRun = [testName];
    }
